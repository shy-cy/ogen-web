...
// ⚠ AND THE FIRST ROW IS A REAL GROUP NOW. It was "Everyone" — a visible row
// standing for the activity's own timetable, which every unnamed group followed
// — and that implicit default is what the restructuring removed: a group is a
// group from the start, renamable, and what decides whether a family is ASKED is
// the COUNT rather than whether anybody typed a name.
H.ok(!/label: 'Everyone'/.test(adminJs), 'there is no implicit default row left to explain');
H.ok(/text: '\+ Add a group'/.test(adminJs), 'a second group is added to a list that already has one');
H.ok(/function primeGroups/.test(adminJs),
  'and the model is primed before any panel draws, because the panel draws the group list ' +
  'and the price preview inside it reads a group\'s session length');
...
H.ok(/facts\.duration = Object\.assign\(\{\}, facts\.duration, \{ sessionDates: e\.dates/.test(adminJs),
  'the calendar is committed from the sub-page\'s working copy onto the group it belongs to');
