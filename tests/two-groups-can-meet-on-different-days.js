...
H.ok(/text: 'Schedule & dates \\u2192'/.test(adminJs), 'from a button on the named-groups list');
H.ok(/function drawGroupPage/.test(adminJs), 'which draws the schedule and the calendar');
H.ok(/function scheduleRowBox\(opts\)/.test(adminJs),
  'built from ONE row builder, shared with the activity\'s own schedule editor');
H.ok(/prefix: GROUP_PREFIX/.test(adminJs), 'namespaced so two editors cannot collide');
H.ok(/action: 'sessions'/.test(adminJs.slice(adminJs.indexOf('function generateGroupSessions'))),
  'and generated by the same server action the activity\'s calendar uses');
// ⚠ The read-back trap, in its newest costume: the list draws neither field, so
// a sync that returned only what it can see would delete both every time a group
// is added or removed.
const sync = adminJs.slice(adminJs.indexOf('function syncNamedGroups'), adminJs.indexOf('function readFacts'));
H.ok(/if \(g\.schedule\) out\.schedule = g\.schedule;/.test(sync),
  'the list read-back CARRIES the schedule it does not draw');
H.ok(/if \(g\.sessionDates\) out\.sessionDates = g\.sessionDates;/.test(sync), 'and the calendar');
H.ok(/commitGroupEdit\(\);\s*\n\s*var facts = readFacts\(\);/.test(adminJs),
  'and a group left open is committed on save, so what is on screen is what is stored');
