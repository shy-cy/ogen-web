...
(async () => {
  ...
  H.eq(keys.length, 3, 'all three uploads are in the map');
  H.ok(keys.indexOf('/images/activities/purim-workshop-hero.png') !== -1, 'the hero is there');
  ...
  // This is the assertion that would have failed before the fix: a src in the
  // markup that neither exists in the repo nor has a data URL to stand in.
  const html = prev.body.html.he;
  const srcs = (html.match(/src="\/images\/activities\/[^"]+"/g) || [])
    .map((m) => m.slice(5, -1));
  H.ok(srcs.length > 0, 'the hero is referenced by path in the HTML, not inlined');
  srcs.forEach((src) => H.ok(!!map[src], src + ' can be shown in the preview'));
  ...
  H.ok(html.indexOf('data:image') === -1, 'no data URL is inlined into the rendered page');
  H.ok(html.indexOf('/images/activities/purim-workshop-hero.png') !== -1, 'the hero is the real path');
  ...
  H.ok(github._files.has('images/activities/purim-workshop-hero.png'), 'the hero file is committed');
  ...
  const again = await H.call(fn.handler, Object.assign({
    action: 'preview',
    activity: Object.assign(activity(), { heroImage: '/images/activities/purim-workshop-hero.png',
                                          teachers: [], sponsors: [] })
  }, auth));
